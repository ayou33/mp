...
src/indicators/custom/
  ...
  formula.ts                  公式 DSL:tokenize → 递归下降解析 AST(parseFormula 校验)→ evaluateFormula 求值
                              FormulaError(带位置) + FORMULA_FIELDS/FORMULA_FUNCS 帮助清单
                              parseFormulaScript(多语句脚本)→ evaluateFormulaScript(变量引用求值);parseFormulaExpr(允许变量引用的单表达式,供脚本 band 下轨)
  formulaIndicator.ts         公式 → CustomIndicatorDef:defineFormulaIndicator(spec) 缓存 AST;`NAME := EXPR` 私有变量只计算不渲染;outputSpecs 支持 label/scale/visible(显示名/独立轴/可见性)
                              FormulaShape / FormulaOutputSpec;FORMULA_PALETTE 脚本各线取色;outputSpecs 决定脚本各输出形态
  userFormulas.ts             用户公式记录注册表:localStorage mp_custom_formulas 持久化(outputSpecs 脚本各输出形态),
...
2. **多输出脚本**(任一语句含 `=` 或 `:=`):每行 `NAME = EXPR`(换行或 `;` 分隔)定义**一条输出**;`NAME := EXPR` 定义**私有中间变量**(如 `MID := SMA(C,20)`)——参与计算、可被后续行/band 下轨引用,但**不渲染为输出**。EXPR 可引用前面行的 `NAME`(如 MACD 的 DIF/DEA 组合)。**每条输出可独立选择形态**(line / area / histogram / baseline / band),由 `outputSpecs`(key = 小写输出名)记录:band 需给 `lower` 下轨公式(可引用脚本变量),baseline 可给 `baseValue` 基准值;线样式按线独立调色/线宽/线型,色取自 `FORMULA_PALETTE`(弹窗内可按线覆盖)。**每条输出还可独立定义**:显示名 `label`(缺省 = 名称大写)、Y 轴 `scale`(主轴/独立轴,独立轴共用 id=`{id}_scale`)、可见性 `visible`(false = 不渲染/不进图例轴标签,但仍参与计算可被引用);缺省 label=名称大写、scale=主轴、visible=true。无 `outputSpecs` 的旧记录按全部 line 渲染(向后兼容);全私有脚本在构建期报错。

**脚本约束**(`parseFormulaScript` 校验):
- 名称 `NAME` 需符合 `[A-Za-z_][A-Za-z0-9_]*`;**不能是保留字**(字段名及简写 c/o/h/l/v、函数名 sma/ema/macd/...)且不能重复;赋值后不允许出现裸表达式(必须每行都带 `=`/`:=`);引用**未定义/前向引用**变量报「未知变量」。`NAME := EXPR` 的语句是**私有中间变量**:顺序求值进入变量表、可被后续行引用,但不进入 `def.outputs`(渲染层不建 series)。
...
`outputs` 元数据(编辑面板线样式列表)必须与 `calc` 返回的 Output **key 对齐**;每个输出默认 Y 轴 `scale`(缺省 `kind:'right'` 与同 pane 主轴共用)。
...
- 公式记录(公式文本/形态/名称 + 脚本各输出形态 `outputSpecs`)持久化在 **`mp_custom_formulas`**(`UserFormulaRecord`),与实例配置 `mp_indicator_config` 分离。删除公式记录即注销定义;仍存在于配置里的实例会因定义缺失而跳过渲染。
- 值级函数输出 `(number|null)[]`,`null` = 无效/预热期;`ctx.points()` 负责过滤与时间对齐。**不要**直接向 series 喂含 null 的数组。
- 渲染层所有颜色默认走 def 的 `outputs[].color` 或类型缺省(红涨绿跌等),用户编辑面板覆盖进 `lineStyles`。
