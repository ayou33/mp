...
- `src/indicators/` — 指标子系统:`IndicatorController`(`IndicatorConfig` 开关/参数/`lineStyles` 每线样式 + 主图默认色常量,装配主图 MA/EMA/BBI/BOLL + 副图 series、十字光标图例分流 `ChartLegend{ohlcv, indicators}`)、`SubChartIndicator.ts`(单个副图指标实例,应用参数与线样式覆盖)、`IndicatorAxis.ts`(指标值价格轴标签 + 副图左上角角标 primitive)、`subCharts.ts`(副图指标定义 RSI/MACD/KDJ/WR/CCI/OBV/ATR/DMI)、`editorMeta.ts`(`INDICATOR_META` 编辑面板元数据:参数 + 输出线)、内置指标纯函数(calcMA/calcMACD/... 见 `@mp/shared`)、`custom/`\(自定义指标框架:公式 DSL 引擎(`formula.ts`/`lib.ts` 等已在 `@mp/shared`,web 从此导入)+ `userFormulas.ts` localStorage 持久化 + `CustomIndicatorManager`/`CustomIndicatorInstance` 渲染器,支持 7 种输出形态 line/area/baseline/histogram/candlestick/bar/band 与主图叠加/副图 pane/多 Y 轴\)。详细约定与新指标扩展路径见 `src/indicators/AGENTS.md`(自定义指标见 `src/indicators/custom/AGENTS.md`)
...
- `src/data/stocks.ts` — 常用 A 股清单 + 指数清单(默认自选三大指数:上证指数/科创综指/创业板指)与名称映射、`BrowseEntry` 浏览记录条目(自选/最近浏览用)
- `src/components/` — React 层(渲染 + 事件接线):App(编排)、KLineChart(图表壳)、IndicatorBar(顶部指标区)、DrawToolbar(左侧画线栏)、Sidebar(右侧自选/浏览)、PriceInput(可复用价格输入:滚轮 1/10/100 tick、精确 0.01,画线编辑/文本标注/操作线创建共用)、`topbar/`(TopBar + StockSearch + SettingsDialog,`UserSettings` 持久化到 `mp_settings`)、`modal/`(ModalProvider 全局弹窗系统 + BaseModal 统一弹窗基础外壳,所有弹窗/浮层由此衍生)、`mobile/`(移动端底部操作栏,小屏 <lg 默认收起指标/画线/自选,仅显示绘图区域)。详细约定与弹窗/样式坑见 `src/components/AGENTS.md`
