...
import type { DrawingKind, DrawingRef, SerializedDrawing } from './types'
...
export abstract class DrawingTool {
  ...
  private readonly _getBarCount: () => number
  ...
  /** 设置只读标记 */
  abstract setReadonly(ref: DrawingRef, v: boolean): void
  /** 查询是否只读 */
  abstract isReadonly(ref: DrawingRef): boolean
  /** 读取控制点价格(菜单输入框显示) */
  abstract getControlPointPrice(ref: DrawingRef): number | null
  /** 设置控制点价格(readonly 对象不可改) */
